...
def drawpolygon(x_vertex,y_vertex):
	n=len(x_vertex)	
	for i in range(n-1):
		lineBres(int(x_vertex[i]),int(y_vertex[i]),int(x_vertex[i+1]),int(y_vertex[i+1]))
	lineBres(int(x_vertex[n-1]),int(y_vertex[n-1]),int(x_vertex[0]),int(y_vertex[0]))	
